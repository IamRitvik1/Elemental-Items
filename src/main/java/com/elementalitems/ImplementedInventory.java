...
public interface ImplementedInventory extends SidedInventory {
    ...
    default boolean contains(Item item){
        for (int i = 0; i < size(); i++) {
            ...
            if(ItemStack.areItemsEqual(itemStack,getItems().get(i))){
                return true;
            }
        }
        return false;
    ...
    /**
     * Gets the used slots to automation on the side
     *
     * <p>The default implementation returns an array of all slots that are filled.
     */
    ...
    default int[] getFilledSlots()
    {
        ...
        for (int i = 0; i < size(); i++) {
            if (!(getItems().get(i) == ItemStack.EMPTY)) {
                result.add(i);
            }
        }
        return Arrays.stream(result.toArray(new Integer[0])).mapToInt(value -> value).toArray();
    }
